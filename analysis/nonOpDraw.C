		...
		char* suffix = "coll_SC")
{
	...
	*/
	...
	// HIST 0 2-D Time - Edep distribution
	if(Opt[0])
	{
		TCanvas* c0 = new TCanvas("c0","c0",1.2*400*4,400*4);
		c0 -> Divide(4,4);
		for(int a=0; a<nSC; a++)
		{
...
//				gPad -> SetLogy();
				TH2F* H2 = 
					(TH2F*) F -> Get(Form("H2_Time_Edep_%s_%s",str_SC[a],str_pID[b]));
				H2 -> Rebin2D(10,2);
				H2 -> SetStats(false);
//				if(b==0)
...
//				H2 -> GetXaxis() -> SetRangeUser(0,10);
				...
				H2 -> GetYaxis() -> SetTitle("Edep [MeV]");
				H2 -> GetYaxis() -> SetTitleSize(0.06);
				H2 -> GetYaxis() -> SetLabelSize(0.045);
				H2 -> SetLineColor(b==2?b+4:b+1);
//				H2 -> SetLineWidth(2);
//				leg -> AddEntry(H2,Form("%s, Mean: %.2f, StdDev: %.2f",str_pID[b],H2->GetMean(),H2->GetStdDev()),"l");
	...
	if(Opt[1])
	{
		...
		for(int a=0; a<nSC; a++)
		{
			...
			for(int b=0; b<npID; b++)
			{
				...
				gPad -> SetLogy();
				TH1F* H1 = 
					(TH1F*) F -> Get(Form("H1_EdepSum_%s_%s",str_SC[a],str_pID[b]));
				H1 -> SetStats(false);
				H1 -> Rebin(10);
				if(b==0)
				{
